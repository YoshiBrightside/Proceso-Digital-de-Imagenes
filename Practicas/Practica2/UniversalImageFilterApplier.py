    ...
    def display_filter_options(self):
        for o in (self.selected_filter.options.keys()):
            ...
            if self.selected_filter.options[o]['style'] == 'dropdown':
                self.filter_options.append(ttk.Combobox(self.option_frame))
                self.filter_options[-1]['values'] = self.selected_filter.options[o]['values'] #self.selected_options[o]
                self.filter_options[-1].bind('<<ComboboxSelected>>', lambda x: self.selected_options[o].set(self.filter_options[-1].get()))
            if self.selected_filter.options[o]['style'] == 'text_box':
                self.filter_options.append(Text(self.option_frame, height='2', width='20'))
                self.filter_options[-1].insert('end', o)
                self.filter_options[-1].bind('<FocusOut>', lambda o=o: print(self.filter_options[o].get(1.0, "end-1c")))#self.selected_options[o].set(self.filter_options[-1].get("1.0",END)))
            if self.selected_filter.options[o]['style'] == 'spinbox':
                self.filter_options.append(Spinbox(self.option_frame, from_=0, to=255))
                self.selected_options[o] = self.filter_options[-1].get('0')
